...
// 	w.show(~0); // show all messages
	...
	pwmBaxis5.setFrequency(25000);
 	...
 	// Watchdog
	FlinkWatchdog watchdog("watchdog", &onBoard, ON_BOARD_WATCHDOG_ID,0.5); // is limited to approximately 450ms
	...
	LaserScannerControl laserControl(dtLaser);
	ControlSystemWithLaser controlSys(dt,laserControl.transDataBlock); 
	
	
	// create safty system
	log.info() << "create safty System";
	OmniSafetyPropertiesWithLaser properties(&controlSys, watchdog);
...
// 		if(slip > 100.0) {
// 			log.trace() << "Slips: "<< slip;
// 		}
		sleep(1);
	}
		
	log.trace() << "doControlStop";
	safetySystem.triggerEvent(event::doControlStop);

	laserControl.stop();
	safetySystem.shutdown();

	log.trace() << "laserControlstop";
