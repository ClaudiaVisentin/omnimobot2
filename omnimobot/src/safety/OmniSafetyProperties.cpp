...
OmniSafetyProperties::OmniSafetyProperties(ControlSystem* cs, FlinkWatchdog& motorBoardWatchdog) :  controlSys(cs), watchdog(motorBoardWatchdog),counter(1000)
{ 
	...
	levels =
	{
		{ level::shutDown, 		         "End of programm", 										      }, 
		{ level::off, 			         "Off state", 												      },       
	...
	level( level::off                     ).addEvent( event::doShutDown,               level::shutDown,                eeros::safety::kPublicEvent);                                                                                                         
	...
	addEventToLevelAndAbove(level::ready, event::doEmergency, level::emergency, eeros::safety::kPublicEvent); 							 // Add the event doEmergency from ready

	
	//############ Define input states and events for all levels ############ 

	level(level::shutDown               ).setInputActions( { ignore(emergencyReset),  ignore(approval) ,  ignore(emergencyStop) 						   });
	...
	level(level::shutDown                ).setOutputActions({set(powerOnLED, false) , set(emergencyLED, false), leave(watchdog)    , set(pwmA0, 0.0), set(pwmA1, 0.0), set(pwmA2, 0.0), set(pwmA3, 0.0), set(pwmA4, 0.0), set(pwmA5, 0.0), set(pwmB0, 0.0), set(pwmB1, 0.0), set(pwmB2, 0.0), set(pwmB3, 0.0), set(pwmB4, 0.0), set(pwmB5, 0.0)}); 
	...
	level(level::ready).setLevelAction([&](SafetyContext* privateContext) {
		...
		if(approval->get() && emergencyStop->get()) {
			...
			if(!controlSys->isaxisHomed() || !controlSys->isReadyToDrive()){
				...
				usleep(2100);
				std::cout<< "einmalig nach 2100:  " << "sumphi ist phix: "<< controlSys->pendulumControl.SumPhi.getIn(1).getSignal().getValue()(0) << "   sumphi soll phix: "<< controlSys->pendulumControl.SumPhi.getIn(0).getSignal().getValue()(0) << "  out accX: "<< controlSys->pendulumControl.aPhi_To_axy.getOutAccxy().getSignal().getValue()(0) << "  out integrator: "<< controlSys->pendulumControl.integrator.getOut().getSignal().getValue()(0)<< "  in pos odo: "<< controlSys->pendulumControl.getInOdometry().getSignal().getValue()(0)<< "  out tip ist: "<< controlSys->pendulumControl.phi_To_Tip.getOutTip().getSignal().getValue()(0) << "  in tip soll: "<< controlSys->pendulumControl.getInPosTip().getSignal().getValue()(0)<<std::endl;
	...
	level(level::driveJoystick).setLevelAction([&](SafetyContext* privateContext) {
		...
		if(controlSys->joystick.startMeasuring() && first) {
			controlSys->measuringVeloIn.startLog();
			controlSys->measuringVeloOut.startLog();
			controlSys->pendulumControl.measuringTipDes.startLog();
			controlSys->pendulumControl.measuringPosActual.startLog();
			first = false;
		}
...
// 			if(!controlSys->isReadyToPendulum()){
// 				privateContext->triggerEvent(event::doEmergency);
// 			}
// 			else {
// 		
...
// 			}
	...
	level(level::driveJoystickPendulum).setLevelAction([&](SafetyContext* privateContext) {
		...
		if(controlSys->joystick.startMeasuring() && first) {
			controlSys->pendulumControl.measuringTipActual.startLog();
			controlSys->pendulumControl.measuringTipDes.startLog();
			controlSys->measuringPhiHall.startLog();
			controlSys->measuringPhides.startLog();
			controlSys->measuringVeloIn.startLog();
			controlSys->measuringVeloOut.startLog();
			first = false;
		}
...
// 			privateContext->triggerEvent(event::stop);
			...
			if (controlSys->jacobi.getOutVL().getSignal().getValue()(0) <= 0.005 && controlSys->jacobi.getOutVL().getSignal().getValue()(1) <= 0.005 && controlSys->jacobi.getOutVL().getSignal().getValue()(2) <= 0.005) {
				usleep(1000);
				controlSys->allAxisStopped();
				std::cout << "   lost stick  " <<controlSys->hallSensorInput.isStickConnected()<<  std::endl;
				stopDriveLED->set(false); 
...
OmniSafetyProperties::~OmniSafetyProperties() { }
