...
class Gaintest : public PeriodicThread {
	public:
		...
		PeriodicThread(period, 0, true, paused){
			...
			Gain.getIn().connect(value.getOut());
		}
		...
		void run() {
			...
			// time: 50000*period
			if(ctr == 50000){ 
				std::cout << "counter true nach: "<< ctr <<"  value 5: "<< Gain.getOut().getSignal().getValue()(5)<<std::endl ;
				end = true;
				stop();
			}
	...
	private:
		eeros::control::Gain<eeros::math::Vector<6>, double> Gain;
